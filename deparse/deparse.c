...
PG_FUNCTION_INFO_V1(deparse);
PG_FUNCTION_INFO_V1(deparse_trigger_data);
...
// void pg_query_deparse_protobuf(PgQueryProtobuf parse_tree)
// {
...
Datum
deparse(PG_FUNCTION_ARGS)
{
  StringInfoData str;
  // StringInfoData parentStr;
	// CollectedCommand *parentCmd;
  ...
  deparseStmt(&str, cmd->parsetree);

  // elog(WARNING,"---------");
  // elog(WARNING,"deparse: deparse: %s",str.data);

  // if (cmd->parent) {
  //   initStringInfo(&parentStr);

  //   deparseStmt(&parentStr, cmd->parent->parsetree);

  //   elog(WARNING,"deparse: parent: %s",parentStr.data);
  // }
...
Datum
deparse_trigger_data(PG_FUNCTION_ARGS)
{
  StringInfoData str;
  ...
  // else
  // appendStringInfoString(out, "{\"xid\":null");
  ...
  // pfree(lsn_str);
	...
	appendStringInfoString(&out, "\"");
	appendStringInfoString(&out, str.data);
	appendStringInfoString(&out, "\"");


  LogLogicalMessage("Schema", out.data, out.len,
            true);
